...
"""
# Modificado: \mathbb -> \mathbbm, self._print -> self._uprint
...
# Modificado ini
    def _uprint(self, expr):
        if self._settings['units']:
            if hasattr(expr, "base"):
                if isinstance(expr.base, Unit):
                    _tex = "1 \\;"
                else:
                    _tex = ""
...
# Modificado ini
        def key_to_sort(x):
            ...
            if x.is_Mul:
                ...
                if hasattr(x, "base") and hasattr(x, "exp"):
                    ...
                    nam += tmp
            return deg, sub, nam
        if self._default_settings['sorted']:
            terms = sorted(expr.args, key=key_to_sort)
# Nodificado fin
        elif self.order == 'none': # Modificado if -> elif
            terms = list(expr.args)
        else:
            terms = self._as_ordered_terms(expr, order=order)
        tex = self._uprint(terms[0])

        for term in terms[1:]:
            if not _coeff_isneg(term):
                tex += " + " + self._uprint(term)
            else:
                tex += " - " + self._uprint(-term)
...
# Modificado ini
        ...
        num_units = []
        if (self._settings['units'])&(expr.is_Mul):
#                    args = sorted(args, key = lambda x: isinstance(x, Unit))
            values = []
            den_units = []
            for arg in expr.args:
                if hasattr(arg, "base"):
                    if isinstance(arg.base, Unit):
                        if arg.exp < 0:
                            if arg.exp == -1:
                                den_units.append(str(arg.base))
                            else:
                                den_units.append(str(arg.base) + "^{" + str(-arg.exp) + "}")
                        else:
                            if arg.exp == 1:
                                num_units.append(str(arg.base))
                            else:
                                num_units.append(str(arg.base) + "^{" + str(arg.exp) + "}")
                    else:
                        values.append(arg)
                else:
                    if isinstance(arg, Unit):
                        num_units.append(str(arg))
                    else:
                        values.append(arg)
            if (num_units == [])&(den_units != []):
                num_units.append("1")
            expr = Mul(*values)
# Modificado fin
        ...
        def convert(expr):
            if (not expr.is_Mul)&(num_units == []): # Modificado: and num_units == []
...
# Modificado ini
        if (self._settings['units'])&(num_units != []):
            if tex == "" or tex == "- ":
                tex += "1 "
            if den_units != []:
                tex += " \\;\\ufrac{" + "\\,".join(num_units) + "}{" + "\\,".join(den_units) + "}"
            else:
                tex += " \\;" + "\\,".join(num_units)
# Modificado fin
    ...
    def _print_MatrixBase(self, expr):
        ...
        for line in range(expr.rows):  # horrible, should be 'rows'
            lines.append(" & ".join([ self._uprint(i) for i in expr[line, :] ]))
    ...
    def _print_list(self, expr):
        return r"\begin{bmatrix}%s\end{bmatrix}" % \
            r", & ".join([ self._uprint(i) for i in expr ])
    ...
    def _print_dict(self, d):
        ...
        for key in keys:
            val = d[key]
            items.append("%s : %s" % (self._print(key), self._uprint(val)))
    ...
    def _print_FiniteField(self, expr):
        return r"\mathbbm{F}_{%s}" % expr.mod
